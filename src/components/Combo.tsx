...
export const Card = ({ title, subTitle, content, mark, position = 'left', date, className, selected }: CardProps) => {
  return (
    <div className={`text-${position === 'left' ? 'right' : 'left'} ${position === 'right' ? 'ml-auto' : ''} ${className}`}>
...
const Combo = ({ combo }: { combo: Combo }) => {
  ...
  return (
    <div>
      <Card
        title={combo.name}
        subTitle={`${nutrients.carb}g|${nutrients.protein}g|${nutrients.fat}g`}
